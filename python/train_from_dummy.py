...
def transliterate_sentence(sentence, dic=translation):
    #tokens = word_tokenize(sentence, return_tokens=True)
    tokens = [sentence]
    proc_tokens = []
    for token in tokens:
        if token in punct: proc_tokens.append(token); continue
...
f.close()
data_input = list(map(transliterate_sentence, data_input))
