...
parser.add_argument(
    "-m", "--model", help="Model architecture: transformer, rnn", default="transformer")
...
if model_name == 'transformer':
    ...
    raise Exception(f'Model type {model_name} is not presented in the list of possible models!')
...
if model_name == 'rnn':
    hyperparameters.update({
        'input_dim': len(input_vocab_counter),
        'output_dim': len(target_vocab_counter)
    })
...
#     out = model(sample['input_data'].to(device), sample['target_data'].to(device))
#     print(out.shape)
...
#      exit()
    ...
    encoder_model = Encoder(**hyperparameters).to(device)
    decoder_model = Decoder(**hyperparameters).to(device)
    model = Seq2Seq(encoder_model, decoder_model, device).to(device)
    
#     sample = next(iter(train_dl))
#     outputs = torch.LongTensor([[latn_vocab["<sos>"]]]*sample['input_data'].shape[0]).T
#     out = model(sample['input_data'].T.to(device), outputs.to(device), 0)
#     print(outputs.shape, out.shape)
#     outputs = torch.cat((outputs.T.to(device), out), 1).T
#     print(outputs.shape)
#     out = model(sample['input_data'].T.to(device), outputs, 0)
#     print(out.shape)
#     print(hyperparameters)
#     print(out)    
#     exit()
    ...
    for i in range(max_length):
        ...
        with torch.no_grad():
            if model_name == 'transformer':
                ...
                output = model(sentences_tensor, trg_tensor, 0.0)
